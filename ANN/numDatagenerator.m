clear;
%%
max_eta = 5; grid_size = 0.05; eta = 0:grid_size:max_eta;

% alpha = [-1.78:0.01:-0.8 -0.8:0.05:3];
alpha = alpha1_EO;
M = [0.5];
m0 = [-0.5];
n0 = [-0.3];
...
[d2f01_EO, dg01_EO, dtheta01_EO] = hnfSysSolve(alpha, n0, m0, M, phi, grid_size, max_eta);
% d2f0_water = d2f0(:, 1:3); dg0_water = dg0(:, 1:3); dtheta0_water = dtheta0(:, 1:3); 
% d2f0_EO = d2f0(:, 1:3); dg0_EO = dg0(:, 1:3); dtheta0_EO = dtheta0(:, 1:3);

function [d2f0, dg0, dtheta0]= hnfSysSolve(alpha, n0, m0, M, phi, grid_size, max_eta)
    ...
    fluid_water = [997.1 4179 0.000891 0.613 5.5e-6 6.3];
    ...
    d2f0 = []; dg0 = []; dtheta0 = [];
    i = 1; 
    for fluid_properties = [fluid_EO]' % loop for base fluids
        % if i == 1
                ...
                for n0_in = n0
                    for phi_in = phi
                        ...
                        for alpha_in = alpha
                            ...
                            d2f0(count, i) = d2f; dg0(count, i) = dg; dtheta0(count, i) = dtheta;
                            count = count + 1; % proceed to next row
                        end
                        i = i+1; % next column %n0 % phi
                    end
